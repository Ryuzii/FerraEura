...
// Apple Music autoplay handler (amAutoPlay)
async function amAutoPlay(seedId, storefront = 'us') {
    try {
        // Apple Music API is not public, but we can use the iTunes RSS feed for recommendations
        // This is a best-effort approach for demo purposes
        // For a real implementation, use a proper Apple Music API wrapper with a developer token
        const url = `https://itunes.apple.com/lookup?id=${seedId}&entity=song&country=${storefront}`;
        const data = await fetchPage(url);
        const json = JSON.parse(data);
        if (!json.results || json.results.length < 2) throw new Error('No recommended Apple Music tracks found.');
        // The first result is the seed track, the rest are related tracks
        const related = json.results.slice(1);
        if (!related.length) throw new Error('No related Apple Music tracks found.');
        const track = related[Math.floor(Math.random() * related.length)];
        return track.trackId || track.collectionId || null;
    } catch (err) {
