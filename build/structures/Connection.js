class Connection {
    ...
    setServerUpdate(data) {
        ...
        this.player.tera.emit("debug", `[Player ${this.player.guildId} - CONNECTION] Voice server update: ${previousVoiceRegion !== null ? `Region changed from ${previousVoiceRegion} to ${this.region}` : `Connected to ${this.region}`}`);
        // Auto-resume if player was paused during connection
        if (this.player.paused && this.connectionState === 'connecting') {
            this.player.tera.emit("debug", this.player.node.name, `Auto-resuming ${this.player.guildId} player after connection`);
    ...
    setStateUpdate(data) {
        ...
        this.player.tera.emit("debug", `[Player ${this.player.guildId} - CONNECTION] Voice state update: ${channel_id !== null ? `Connected to ${this.voiceChannel}` : `Disconnected from ${this.voiceChannel}`}`);
        // Handle disconnection
        if (channel_id == null) {
            this.connectionState = 'disconnected';
            this.player.connected = false;
            this.player.tera.emit("playerDisconnect", this.player);
            this.player.destroy();
            return;
        }
        // Handle channel move
        if (this.player.voiceChannel && channel_id && this.player.voiceChannel !== channel_id) {
            this.player.tera.emit("playerMove", this.player.voiceChannel, channel_id);
    ...
    async processUpdateQueue() {
        ...
        try {
            ...
            this.player.tera.emit("debug", `[Connection] Failed to process update queue: ${error.message}`);
    ...
    async updatePlayerVoiceData(updateData = null) {
        ...
        this.player.tera.emit("debug", this.player.node.name, `[Rest Manager] Updating player with voice data: ${JSON.stringify({ voice: data.voice })}`);
        try {
            ...
            this.player.tera.emit("debug", `[Connection] Failed to update player: ${error.message}`);
            throw error;
        }
    }
