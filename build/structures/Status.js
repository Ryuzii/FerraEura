...
class Status {
    ...
    _handleError(error, method) {
        if (error.response) {
            console.error(`[Status] API Error in ${method} (${error.response.status}): ${error.response.data.message}`);
        } else if (error.request) {
            console.error(`[Status] No response received from API in ${method}`);
        } else {
            console.error(`[Status] Request Setup Error in ${method}: ${error.message}`);
        }
    }
}
